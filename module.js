...
function Screenshot(args) {
  ...
  switch (process.platform) {
    case "win32":
      platformJS = require("./capture/win32.js");
      break;
    case "linux":
      platformJS = require("./capture/linux.js");
      break;
    case "darwin":
      platformJS = require("./capture/darwin.js");
      break;
  }
  ...
  if (!platformJS) {
    ...
    platformJS(config.options, function(error, options) {
  ...
  function handleCallback(error, success) {
    if (typeof config.callback === "function") {
      if (typeof success === "undefined") success = !error;
...
Screenshot.prototype.processImage = function(input, output, options, callback) {
  if (
    ...
    new jimp(input, function(err, image) {
      ...
      if (typeof resWidth === "number" && typeof resHeight !== "number")
        // resize to width, maintain aspect ratio
        var resHeight = Math.floor(
          image.bitmap.height * (resWidth / image.bitmap.width)
        );
      else if (typeof resHeight === "number" && typeof resWidth !== "number")
        // resize to height, maintain aspect ratio
        var resWidth = Math.floor(
          image.bitmap.width * (resHeight / image.bitmap.height)
        );
    ...
    if (args.hasOwnProperty(property)) {
      switch (typeof args[property]) {
        ...
        case "function":
          ...
          break;
        case "object":
          if (args[property] != null) config.options = args[property];
          break;
      }
    }
  }

  if (typeof file === "string") config.options.output = file;
